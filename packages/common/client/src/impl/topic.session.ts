...
import { BadRequestExecption, IReadableStream, Packet, PacketBuffer, RequestPacket, StatusVaildator, StreamAdapter, TopicClient, TopicMessage, TransportOpts, TransportRequest } from '@tsdi/common';
...
export class ClientTopicTransportSessionFactory implements ClientTransportSessionFactory<TopicClient> {
    ...
    create(socket: TopicClient, options: TransportOpts): ClientTopicTransportSession<TopicClient> {
        return new ClientTopicTransportSession(this.injector, socket, this.statusVaildator, this.streamAdapter, this.encoder, this.decoder, new PacketBuffer(), options);
    }
