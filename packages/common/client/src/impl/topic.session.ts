import { Injectable, Injector, Optional, isDefined, isString, promisify } from '@tsdi/ioc';
import {
    BadRequestExecption, MimeAdapter, Packet, RequestPacket, ResponseEventFactory, StatusAdapter,
...
export class ClientTopicTransportSession<TSocket extends TopicClient = TopicClient> extends ClientEventTransportSession<TSocket, TopicMessage> {
    ...
    protected async beforeRequest(packet: TransportRequest): Promise<void> {
    ...
    protected getReply(req: TransportRequest) {
        if(isString(req.pattern)) return req.pattern + '/reply'
        return req.pattern.replyTo || req.pattern.topic + '/reply';
    }
