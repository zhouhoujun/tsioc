...
export const requestServializeBackend: BackendFn<AbstractRequest<any>> = (input: AbstractRequest<any>, context: TransportContext) => {
...
export const requestServializeBodyOnlyBackend: BackendFn<AbstractRequest<any>> = (input: AbstractRequest<any>, context: TransportContext) => {
    if (context.transport.streamAdapter.isJson(input.body)) {
        return of(JSON.stringify(input.body));
    }
