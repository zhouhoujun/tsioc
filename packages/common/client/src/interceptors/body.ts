...
import {
    isArrayBuffer, isBlob, isBuffer, isFormData, IStream, isUrlSearchParams,
...
export class BodyContentInterceptor<TRequest extends TransportRequest = TransportRequest, TResponse = TransportEvent> implements Interceptor<TRequest, TResponse> {
    ...
    intercept(req: TRequest & RequestSerialize, next: Handler<TRequest, TResponse>): Observable<TResponse> {
        ...
        if (!session.outgoingAdapter) return next.handle(req);

        const outgoingAdapter = session.outgoingAdapter;
        ...
        return defer(async () => {
            ...
            if (!outgoingAdapter.hasContentLength(req)) {
                ...
                contentLength = Buffer.byteLength(body as Buffer);
            }

            return outgoingAdapter.updateContent(req, body, { contentType, contentLength });
