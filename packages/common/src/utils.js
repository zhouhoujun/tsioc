...
exports.XSSI_PREFIX = exports.htmlRegExp = exports.xmlRegExp = exports.LOCALHOST = void 0;
 ...
 * @returns
 */
function normalize(route, prefix, noSearch) {
    if (!route)
        return '';
...
function escapeHtml(content) {
    const str = '' + content;
    const match = exports.htmlRegExp.exec(str);
    if (!match) {
        return str;
    }
    let escape;
...
function append(header, field) {
    if (typeof header !== 'string') {
        throw new ioc_1.ArgumentException('header argument is required');
    }
    if (!field) {
        throw new ioc_1.ArgumentException('field argument is required');
    }
    // get fields array
    ...
    for (let j = 0; j < fields.length; j++) {
        if (!field_name.test(fields[j])) {
            throw new ioc_1.ArgumentException('field argument contains an invalid header name');
        }
    }
