...
export const BindMthPointcutAction = function (ctx: RuntimeContext, next: () => void): void {
    ...
    const actpdr = ctx.injector.action();
    const scope = actpdr.get(ProceedingScope);
    ...
    const advicesMap = actpdr.get(ADVISOR).getAdviceMap(targetType);
...
export const BeforeCtorAdviceAction = function (ctx: RuntimeContext, next: () => void): void {
    ...
    ctx.injector.action()
        .get(ProceedingScope)
        .beforeConstr(ctx.type, ctx.params, ctx.args, ctx.injector);
...
export const AfterCtorAdviceAction = function (ctx: RuntimeContext, next: () => void): void {
    ...
    ctx.injector.action()
        .get(ProceedingScope)
        .afterConstr(ctx.instance, ctx.type, ctx.params, ctx.args, ctx.injector);
...
export const MatchPointcutAction = function (ctx: RuntimeContext, next: () => void): void {
    ...
    const acpdr = ctx.injector.action();
    let advisor = acpdr.get(ADVISOR);
    let matcher = acpdr.get(ADVICE_MATCHER);
