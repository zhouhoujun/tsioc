...
- `[x]` `AGENT-M2-ARCH`：定义主 agent / worker agent delegation payload 与 result schema
...
- `[x]` `AGENT-M2-UI`：在 review / tasks 面板展示 worker 数量、状态、摘要与失败原因
...
- `[x]` `AGENT-M3-UI-4`：增加“仅看失败任务”“仅看可回滚任务”，并补齐“仅看新增”过滤
- `[x]` `AGENT-M3-TEST`：补大 diff 渲染性能与交互回归测试
...
- `[x]` `AGENT-M4-UI-2`：让 summary / todo / review 可以按项目主线聚合展示
- `[x]` `AGENT-M4-TEST`：补多主题切换、跨 session 聚合与兼容性测试
