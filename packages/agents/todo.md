...
3. ~~Tier1-6 Plan 只读模式~~（已完成，2026-08：会话级 `planMode` + `/plan` `/status` + gateway RPC + system prompt 提示，agent 381 / agent-ui 246 / gateway 119 全绿）+ Tier1-2 沙箱阶段一（待办：`sandboxExec` 包装器 + `sandboxMode` 配置，接 approval/policy 管线）
