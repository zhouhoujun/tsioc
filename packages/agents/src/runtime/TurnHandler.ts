...
import { AGENT_RUNTIME, AGENT_TURN_BACKENDS, AGENT_TURN_FILTERS, AGENT_TURN_GUARDS, AGENT_TURN_INTERCEPTORS } from '../tokens';
...
export class TurnHandler extends AbstractConfigableHandler<AgentTurnInput, Promise<AgentTurnResult>, RunContext> {
    ...
    constructor(@Inject(ApplicationContext) app: ApplicationContext, @Inject(AGENT_RUNTIME) runtime: AgentRuntime) {
        super();
