...
export class HttpContext extends AssetServerContext<HttpServRequest, HttpServResponse> implements Throwable {
    ...
    //     if (this.request.httpVersionMajor < 2) this.response.statusMessage = this.status.status.message(code);
    //     if (this.body && this.status.status.isEmpty(code)) this.body = null;
    // }
