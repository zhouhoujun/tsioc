...
import { MQTT_CLIENT_OPTS } from './options';
import { MqttClient } from 'mqtt';

@Injectable()
export class MqttRequestAdapter extends RequestAdapter<TransportRequest, TransportEvent, number | string> {
    ...
    send(req: TransportRequest): Observable<TransportEvent> {
        return new Observable((observer: Observer<TransportEvent>) => {
            ...
            const client = context.get(MqttClient, InjectFlags.Self);
            const opts = context.get(MQTT_CLIENT_OPTS);
            ...
            const request = context.get(TransportSessionFactory).create(client, opts.transportOpts);
