...
function getMethodMetadata(decorator, target) {
    ...
    if (!meta || utils.isArray(meta) || Object.keys(meta).length < 0) {
        ...
        if (data.tokenKey && data.targetType) {
            ...
            decorators$$1.forEach(function (decorator) {
                if (factories.hasClassMetadata(decorator, data.targetType)) {
                    ...
                    var metas_1 = factories.getMethodMetadata(decorator, data.targetType);
                    var meta_1;
                    Object.keys(metas_1).forEach(function (n) {
                        var mm = metas_1[n];
                        if (mm && !meta_1) {
            ...
            if (providers.isProviderMap(p)) {
                ...
                if (utils.isBaseObject(p)) {
                    Object.keys(p).forEach(function (name) {
                        if (!utils.isUndefined(p[name])) {
                            if (utils.isClass(p[name])) {
                                map.add(name, p[name]);
                            }
                            else if (utils.isFunction(p[name]) || utils.isString(p[name])) {
