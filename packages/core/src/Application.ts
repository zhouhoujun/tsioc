...
import { ApplicationContext, ApplicationContextFactory, ApplicationOption, EnvironmentOption } from './ApplicationContext';
...
export class Application<T = any> {
    ...
    protected createInjector<T>(providers: Provider[], option: ApplicationOption<T>) {
        ...
        if (option.baseURL) {
            const appArgs = container.get(ApplicationArguments, null);
            if (appArgs) {
