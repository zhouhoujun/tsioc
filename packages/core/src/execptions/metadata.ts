...
import { ServerEndpointContext } from '../transport';
...
export interface ExecptionHandlerMetadata {
     ...
     */
    response?: 'body' | 'header' | 'response' | Type<ExecptionRespond> | ((ctx: ServerEndpointContext, returnning: any) => void)
}
            ...
            decors.forEach(decor => {
                ...
                if (response) {
                    if (isClass(response)) {
                        ...
                        invoker.onReturnning((ctx, value) => {
                            ctx.resolve(ExecptionTypedRespond).respond(ctx.resolve(ServerEndpointContext), response, value);
                        })
                    }
                }
