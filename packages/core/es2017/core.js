...
class MethodAccessor {
    ...
    async invoke(token, propertyKey, target, ...providers) {
        if (!target) {
...
let ModuleInjector = class ModuleInjector extends BaseModuleInjector {
    ...
    async inject(container, modules) {
        let types = this.filter(modules);
        if (types.length) {
            await utils.PromiseUtil.step(types.map(ty => {
...
class ModuleInjectorChain {
    ...
    async inject(container, modules) {
        let types = [];
        await utils.PromiseUtil.forEach(this.injectors.map(jtor => (ijrt) => jtor.inject(container, ijrt.next)), result => {
...
class DefaultContainerBuilder {
     ...
     * @memberof DefaultContainerBuilder
     */
    async loadModule(container, ...modules) {
        ...
        if (regModules && regModules.length) {
            ...
            await utils.PromiseUtil.step(regModules.map(async (typs) => {
