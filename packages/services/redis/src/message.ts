...
import { AbstractServerTransport, IReadableStream, MessageReader, MessageWriter, StreamAdapter, ev, toBuffer } from '@tsdi/common/transport';
...
export class RedisMessageReader implements MessageReader<ReidsSocket, any, RedisMessage> {
    ...
    read(socket: ReidsSocket, channel: any, messageFactory: RedisMessageFactory, session?: AbstractServerTransport): Observable<RedisMessage> {
        return merge(
...
export class RedisMessageWriter implements MessageWriter<ReidsSocket, RedisMessage> {
    ...
    async write(socket: ReidsSocket, msg: RedisMessage, origin: any, session: AbstractServerTransport): Promise<any> {
        if (!msg.data) return;
