...
describe('Nats hybrid Tcp Server & Nats Client & TcpClient', () => {
    ...
    it('nats client fetch json', async () => {
        const res: any = await lastValueFrom(natsClient.send('content/510100_full.json')
            .pipe(
                catchError((err, ct) => {
                    ctx.getLogger().error(err);
                    return of(err);
    ...
    it('nats client fetch big json', async () => {
        const res: any = await lastValueFrom(natsClient.send('content/big.json')
            .pipe(
                catchError((err, ct) => {
                    ctx.getLogger().error(err);
                    return of(err);
