...
export interface KarmaCoverageOptions {
    enabled?: boolean;
...
/**
 * V8CoverageCollector for Node.js V8 coverage data.
 * Collects coverage from NODE_V8_COVERAGE output files.
 */
@Injectable()
export class V8CoverageCollector extends CoverageCollector {
    private fileCoverages: Map<string, FileCoverageData> = new Map();
    private options: KarmaCoverageOptions;
    ...
    constructor() {
        super();
        this.options = {};
        this.coverageDir = path.join(process.cwd(), '.nyc_output');
        this.rootPath = process.cwd();
    }

    setOptions(options: KarmaCoverageOptions, baseURL?: string): void {
        this.options = options || {};
        this.coverageDir = this.options.outputDir || process.env.NODE_V8_COVERAGE || path.join(process.cwd(), '.nyc_output');
        this.rootPath = baseURL || process.cwd();
    }

    async collect(): Promise<void> {
        // Trigger V8 coverage snapshot
        if (typeof (v8 as any).takeCoverage === 'function') {
            try {
                (v8 as any).takeCoverage();
            } catch (e) {
                // Ignore errors from takeCoverage
            }
    ...
    private async readCoverageFiles(): Promise<void> {
        if (!fs.existsSync(this.coverageDir)) {
            console.log(`\nWarning: Coverage directory not found: ${this.coverageDir}`);
            console.log('Make sure to run with NODE_V8_COVERAGE environment variable.');
            return;
        }
        ...
        if (coverageFiles.length === 0) {
            console.log(`\nWarning: No coverage JSON files found in ${this.coverageDir}`);
            return;
        }
    ...
    private processScript(script: V8CoverageScript, source: string, filePath: string): FileCoverageData {
        ...
        for (const func of script.functions) {
            ...
            for (const range of func.ranges) {
                ...
                // Track branch coverage (for block coverage)
