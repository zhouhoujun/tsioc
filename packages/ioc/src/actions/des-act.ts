...
export const RegClassAction = function (ctx: DesignContext, next: () => void): void {
    regProvider(ctx.getRecords(), ctx.platform, ctx.injector, ctx.type, ctx.provide || ctx.type, ctx.singleton || ctx.typeRef.annotation.singleton === true, ctx.typeRef.annotation.static);
    next()
};
...
export const TypeProviderAction = function (ctx: DesignContext, next: () => void) {
    ...
    if (regpdr && regpdr !== type) {
        ctx.typeRef.provides.forEach(provide => {
...
export const IocAutorunAction = function (ctx: DesignContext, next: () => void) {
    const runs = ctx.typeRef.runnables.filter(c => c.auto && c.decorType === Decors.CLASS);
    if (runs.length < 1) {
        return next()
    }
    ...
    if (!instance) return;
    const factory = injector.get(ReflectiveFactory).create(ctx.typeRef, injector);
    runs.forEach(meta => {
