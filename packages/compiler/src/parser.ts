...
import { TagDefinition } from './lexer';
...
export class Parser {
  ...
  parse(source: string, url: string, options?: lex.TokenizeOptions): ParseTreeResult {
    ...
    parser.build();
    return {
        rootNodes: parser.rootNodes,
        errors: (tokenizeResult.errors as ParseError[]).concat(parser.errors),
    };
  }
}
...
class _TreeBuilder {
  ...
  private _elementStack: t.Element[] = [];
  ...
  private _consumeComment(token: lex.Tokenize) {
    ...
    this._addToParent(new t.Comment(value, token.sourceSpan));
  }
  ...
  private _consumeExpansion(token: lex.Tokenize) {
    ...
    this._addToParent(new t.Expansion(
        switchValue.parts[0], type.parts[0], cases, sourceSpan, switchValue.sourceSpan));

    this._advance();
  }

  private _parseExpansionCase(): t.ExpansionCase|null {
  ...
  private _consumeText(token: lex.Tokenize) {
    ...
    if (text.length > 0) {
      this._addToParent(new t.Text(text, token.sourceSpan));
    }
  }
  ...
  private _consumeStartTag(startTagToken: lex.Tokenize) {
    ...
    const startSpan = new ParseSourceSpan(
        startTagToken.sourceSpan.start, end, startTagToken.sourceSpan.fullStart);
    const el = new t.Element(fullName, attrs, [], span, startSpan, undefined);
  ...
  private _pushElement(el: t.Element) {
  ...
  private _consumeAttr(attrName: lex.Tokenize): t.Attribute {
  ...
  private _getParentElement(): t.Element|null {
    return this._elementStack.length > 0 ? this._elementStack[this._elementStack.length - 1] : null;
  }

  private _addToParent(node: t.Node) {
    const parent = this._getParentElement();
    if (parent != null) {
  ...
  private _getElementFullName(prefix: string, localName: string, parentElement: t.Element|null):
      string {
    if (prefix === '') {
