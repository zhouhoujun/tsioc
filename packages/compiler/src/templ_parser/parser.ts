...
import { CompileDirectiveMetadata, CompileDirectiveSummary, CompilePipeSummary, CompileTokenMetadata, CompileTypeMetadata } from '../metadata';
import { CompileReflector } from '../reflector';
import { CompilerConfig } from '../config';
import { SchemaMetadata } from '../core';
import { AST, ASTWithSource, EmptyExpr, ParsedEvent, ParsedProperty, ParsedVariable } from '../exp_parser/ast';
...
import { PreparsedElementType, preparseElement } from './preparser';
...
export class TemplateParser {
  ...
  tryParse(
    ...
    let htmlParseResult = typeof template === 'string' ?
      this._htmlParser!.parse(template, templateUrl, {
        ...
        markers: this.getMarkers(component)
      }) :
      template;
  ...
  tryParseHtml(
    ...
    if (htmlAstWithErrors.rootNodes.length > 0) {
      ...
      const bindingParser = new BindingParser(
        this._exprParser, markers!, this._schemaRegistry, uniqPipes, errors);
  ...
  getMarkers(component: CompileDirectiveMetadata): Markers | undefined {
    if (component.template) {
      return Array.isArray(component.template.markers) ? { start: component.template.markers[0], end: component.template.markers[1] } : DEFAULT_MARKERS;
    }
    return undefined;
  }
...
class TemplateParseVisitor implements html.Visitor {
  ...
  private _parseDirectives(selectorMatcher: SelectorMatcher, elementCssSelector: CssSelector): { directives: CompileDirectiveSummary[], matchElement: boolean } {
