...
export class BinaryOperatorExpr extends Expression {
  ...
  constructor(
    ...
    sourceSpan?: ParseSourceSpan | null, public parens = true) {
...
export function findReadVarNames(stmts: Statement[]): Set<string> {
  const visitor = new ReadVarVisitor();
  visitor.visitAllStatements(stmts, null);
  return visitor.varNames;
}

class ReadVarVisitor extends RecursiveAstVisitor {
...
export function collectExternalReferences(stmts: Statement[]): ExternalReference[] {
  const visitor = new FindExternalReferencesVisitor();
  visitor.visitAllStatements(stmts, null);
  return visitor.externalReferences;
}

class FindExternalReferencesVisitor extends RecursiveAstVisitor {
...
export function applySourceSpanToStatementIfNeeded(
  ...
  const transformer = new ApplySourceSpanTransformer(sourceSpan);
...
class ApplySourceSpanTransformer extends AstTransformer {
