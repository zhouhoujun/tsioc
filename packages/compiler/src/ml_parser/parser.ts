...
export class Parser {
  ...
  parse(source: string, url: string, options?: lex.TokenizeOptions): ParseTreeResult {
    ...
    const treeAndErrors = new TreeBuilder(tokensAndErrors.tokens, this.getTagDefinition).build();
...
class TreeBuilder {
  ...
  private _consumeCdata(startToken: lex.Tokenize) {
    this._consumeText(this._advance());
  ...
  private _parseExpansionCase(): html.ExpansionCase | null {
    ...
    const parsedExp = new TreeBuilder(exp, this.getTagDefinition).build();
