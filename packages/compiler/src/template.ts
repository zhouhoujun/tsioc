import { RenderFlags } from '@tsdi/components';
import * as t from './ast';
import { DEFAULT_MARKERS, LexerRange, Markers, ParseSourceSpan, sanitizeIdentifier } from './util';
...
import { IvyParser } from './exp_parser/parser';
import { Lexer } from './exp_parser/lexer';



// Selector attribute name of `<v-content>`
const CONTENT_SELECT_ATTR = 'select';

// Attribute name of `projectAs`.
const PROJECT_AS_ATTR_NAME = 'projectAs';
...
const GLOBAL_TARGET_RESOLVERS = new Map<string, o.ExternalReference>(
   [['window', Idf.resolveWindow], ['document', Idf.resolveDocument], ['body', Idf.resolveBody]]);

export const LEADING_TRIVIA_CHARS = [' ', '\n', '\r', '\t'];

//  if (rf & flags) { .. }
export function renderFlagCheckIfStmt(
   flags: RenderFlags, statements: o.Statement[]): o.IfStmt {
...
export class TemplateDefinitionBuilder implements t.Visitor<void>, LocalResolver {
   ...
   private _contentReservedSlots: (string | '*')[] = [];

   // Number of non-default selectors found in all parent templates of this template. We need to
   // track it to properly adjust projection slot index in the `projection` instruction.
   private _contentSelectorsOffset = 0;
   ...
   constructor(
      ...
      this._valueConverter = new ValueConverter(
         ...
         (name, localName, slot, value: o.Expression) => {
            ...
            this.creationInstruction(null, Idf.pipe, [o.literal(slot), o.literal(name)]);
         });
   }

   buildTemplateFunction(
      nodes: t.Node[], variables: t.Variable[], contentSelectorsOffset: number = 0,
      i18n?: i18n.I18nMeta): o.FunctionExpr {
      this._contentSelectorsOffset = contentSelectorsOffset;

      if (this._namespace !== Idf.namespaceHTML) {
      ...
      //   but it's not initiated by the Element (e.g. <v-template i18n>)
      ...
      // Output the `projectionDef` instruction when some `<v-content>` tags are present.
      // The `projectionDef` instruction is only emitted for the component template and
      // is skipped for nested templates (<v-template> tags).
      if (this.level === 0 && this._contentReservedSlots.length) {
         const parameters: o.Expression[] = [];

         // By default the `projectionDef` instructions creates one slot for the wildcard
         // selector if no parameters are passed. Therefore we only want to allocate a new
         // array for the projection slots if the default projection slot is not sufficient.
         if (this._contentReservedSlots.length > 1 || this._contentReservedSlots[0] !== '*') {
            const r3ReservedSlots = this._contentReservedSlots.map(
         ...
         this.creationInstruction(null, Idf.projectionDef, parameters, /* prepend */ true);
      }
      ...
      if (isRoot && isResolved && !isEmitted && !isSingleI18nIcu(meta)) {
         ...
         if (needsPostprocessing) {
            transformFn = (raw: o.ReadVarExpr) => {
               ...
               return instruction(null, Idf.i18nPostprocess, args);
            };
         }
   ...
   private i18nStart(span: ParseSourceSpan | null = null, meta: i18n.I18nMeta, selfClosing?: boolean):
      ...
      this.creationInstruction(span, selfClosing ? Idf.i18n : Idf.i18nStart, params);
   }
   ...
   private i18nEnd(span: ParseSourceSpan | null = null, selfClosing?: boolean): void {
      ...
      if (bindings.size) {
         ...
         this.updateInstructionChainWithAdvance(this.getConstCount() - 1, Idf.i18nExp, chainBindings);
         this.updateInstruction(span, Idf.i18nApply, [o.literal(index)]);
      }
      if (!selfClosing) {
         this.creationInstruction(span, Idf.i18nEnd);
      }
   ...
   private i18nAttributesInstruction(
      ...
      if (bindings.length > 0) {
         this.updateInstructionChainWithAdvance(nodeIndex, Idf.i18nExp, bindings);
      }
      if (i18nAttrArgs.length > 0) {
         const index: o.Expression = o.literal(this.allocateDataSlot());
         const constIndex = this.addToConsts(o.literalArr(i18nAttrArgs));
         this.creationInstruction(sourceSpan, Idf.i18nAttributes, [index, constIndex]);
         if (hasBindings) {
            this.updateInstruction(sourceSpan, Idf.i18nApply, [index]);
         }
      }
   }
   ...
   private getNamespaceInstruction(namespaceKey: string | null) {
      switch (namespaceKey) {
         case 'math':
            return Idf.namespaceMathML;
         case 'svg':
            return Idf.namespaceSVG;
         default:
            return Idf.namespaceHTML;
      }
   }
   ...
   visitContent(content: t.Content) {
      ...
      const projectionSlotIdx = this._contentSelectorsOffset + this._contentReservedSlots.length;
      const parameters: o.Expression[] = [o.literal(slot)];

      this._contentReservedSlots.push(content.selector);

      const nonContentSelectAttributes =
         content.attributes.filter(attr => attr.name.toLowerCase() !== CONTENT_SELECT_ATTR);
      const attributes =
         this.getAttributeExpressions(content.name, nonContentSelectAttributes, [], []);
      ...
      this.creationInstruction(content.sourceSpan, Idf.projection, parameters);
      if (this.i18n) {
         this.i18n.appendProjection(content.i18n!, slot);
      }
   }
   ...
   visitElement(element: t.Element) {
      ...
      // Handle styling, i18n, nonBindable attributes
      ...
      // Regular element or v-container creation mode
      ...
      if (createSelfClosingInstruction) {
         this.creationInstruction(
            element.sourceSpan, isNgContainer ? Idf.elementContainer : Idf.element,
            trimTrailingNulls(parameters));
      } else {
         this.creationInstruction(
            element.startSourceSpan, isNgContainer ? Idf.elementContainerStart : Idf.elementStart,
            trimTrailingNulls(parameters));

         if (isNonBindableMode) {
            this.creationInstruction(element.startSourceSpan, Idf.disableBindings);
         }
         ...
         if (element.outputs.length > 0) {
            ...
            this.creationInstructionChain(Idf.listener, listeners);
         }
         ...
         if (inputType === BindingType.Animation) {
            ...
            if (value !== undefined) {
               ...
               if (inputType === BindingType.Property) {
                  ...
                  // class prop
                  this.updateInstructionWithAdvance(elementIndex, input.sourceSpan, Idf.classProp, () => {
                     return [
      ...
      if (propertyBindings.length > 0) {
         this.updateInstructionChainWithAdvance(elementIndex, Idf.property, propertyBindings);
      }

      if (attributeBindings.length > 0) {
         this.updateInstructionChainWithAdvance(elementIndex, Idf.attribute, attributeBindings);
      }
      ...
      if (!createSelfClosingInstruction) {
         ...
         if (isNonBindableMode) {
            this.creationInstruction(span, Idf.enableBindings);
         }
         this.creationInstruction(span, isNgContainer ? Idf.elementContainerEnd : Idf.elementEnd);
      }
   }


   visitTemplate(template: t.Template) {
      const NG_TEMPLATE_TAG_NAME = 'v-template';
      ...
      // find directives matching on a given <v-template> node
      ...
      // local refs (ex.: <v-template #foo>)
      if (template.references && template.references.length) {
         const refs = this.prepareRefsArray(template.references);
         parameters.push(this.addToConsts(refs));
         parameters.push(o.importExpr(Idf.templateRefExtractor));
      }
      ...
      // template definition. e.g. <div *if="showing">{{ foo }}</div>  <div #foo></div>
      this._nestedTemplateFns.push(() => {
         const templateFunctionExpr = templateVisitor.buildTemplateFunction(
            template.children, template.variables,
            this._contentReservedSlots.length + this._contentSelectorsOffset, template.i18n);
         this.constantPool.statements.push(templateFunctionExpr.toDeclStmt(templateName));
         if (templateVisitor._contentReservedSlots.length) {
            this._contentReservedSlots.push(...templateVisitor._contentReservedSlots);
         }
      });

      // e.g. template(1, MyComp_Template_1)
      this.creationInstruction(template.sourceSpan, Idf.templateCreate, () => {
         parameters.splice(
      ...
      // handle property bindings e.g. ɵɵproperty('forOf', ctx.items), et al;
      this.templatePropertyBindings(templateIndex, template.templateAttrs);

      // Only add normal input/output binding instructions on explicit <v-template> elements.
      if (tagNameWithoutNamespace === NG_TEMPLATE_TAG_NAME) {
         ...
         // generate `i18nAttributes` instruction. Note: we generate it only for explicit <v-template>
         ...
         if (template.outputs.length > 0) {
            const listeners = template.outputs.map(
               (outputAst: t.BoundEvent) => ({
                  sourceSpan: outputAst.sourceSpan,
                  params: this.prepareListenerParameter('v_template', outputAst, templateIndex)
               }));
            this.creationInstructionChain(Idf.listener, listeners);
         }
      }
   ...
   visitBoundText(text: t.BoundText) {
      ...
      this.creationInstruction(text.sourceSpan, Idf.text, [o.literal(nodeIndex)]);
   ...
   visitText(text: t.Text) {
      ...
      if (!this.i18n) {
         this.creationInstruction(
            text.sourceSpan, Idf.text, [o.literal(this.allocateDataSlot()), o.literal(text.value)]);
      }
   }
   ...
   visitIcu(icu: t.Icu) {
      ...
      const transformFn = (raw: o.ReadVarExpr) => {
         ...
         return instruction(null, Idf.i18nPostprocess, [raw, mapLiteral(formatted, true)]);
      };
   ...
   getNgContentSelectors(): o.Expression | null {
      return this._contentReservedSlots.length ?
         this.constantPool.getConstLiteral(asLiteral(this._contentReservedSlots), true) :
         null;
   }
   ...
   private templatePropertyBindings(
         ...
         if (input instanceof t.BoundAttribute) {
            ...
            if (value !== undefined) {
               ...
               if (value instanceof Interpolation) {
                  ...
                  // for regular HTML elements, but not applicable for <v-template> (since props act as
      ...
      if (propertyBindings.length > 0) {
         this.updateInstructionChainWithAdvance(templateIndex, Idf.property, propertyBindings);
      }
   }
   ...
   private addAdvanceInstructionIfNecessary(nodeIndex: number, span: ParseSourceSpan | null) {
      if (nodeIndex !== this._currentIndex) {
         ...
         this.instructionFn(this._updateCodeFns, span, Idf.advance, [o.literal(delta)]);
   ...
   private getAttributeExpressions(
      ...
      let projectAsAttr: t.TextAttribute | undefined;

      for (const attr of renderAttributes) {
         if (attr.name === PROJECT_AS_ATTR_NAME) {
            projectAsAttr = attr;
         }
         ...
         if (attr.i18n) {
            ...
            // (e.g. `<div *if title="Hello" i18n-title>`), we want to avoid generating
      ...
      // Keep projectAs next to the other name, value pairs so we can verify that we match
      // projectAs marker in the attribute name slot.
      if (projectAsAttr) {
         attrExprs.push(...getNgProjectAsLiteral(projectAsAttr));
      }
      ...
      const refsParam = flatten(references.map(reference => {
         ...
         this._bindingScope.set(
            ...
            DeclarationPriority.DEFAULT, (scope: BindingScope, relativeLevel: number) => {
               ...
               const refExpr = lhs.set(o.importExpr(Idf.reference).callFn([o.literal(slot)]));
...
const pipeBindingIdentifiers = [Idf.pipeBind1, Idf.pipeBind2, Idf.pipeBind3, Idf.pipeBind4];

function pipeBindingCallInfo(args: o.Expression[]) {
   const identifier = pipeBindingIdentifiers[args.length];
   return {
      identifier: identifier || Idf.pipeBindV,
      isVarLength: !identifier,
   };
}

const pureFunctionIdentifiers = [
   Idf.pureFunction0, Idf.pureFunction1, Idf.pureFunction2, Idf.pureFunction3, Idf.pureFunction4,
   Idf.pureFunction5, Idf.pureFunction6, Idf.pureFunction7, Idf.pureFunction8
];

function pureFunctionCallInfo(args: o.Expression[]) {
   const identifier = pureFunctionIdentifiers[args.length];
   return {
      identifier: identifier || Idf.pureFunctionV,
      isVarLength: !identifier,
   };
}
...
function generateNextContextExpr(relativeLevelDiff: number): o.Expression {
   return o.importExpr(Idf.nextContext)
...
export class BindingScope implements LocalResolver {
   ...
   restoreViewStatement(): o.Statement[] {
      ...
      if (this.restoreViewVariable) {
         const restoreCall = instruction(null, Idf.restoreView, [this.restoreViewVariable]);
   ...
   viewSnapshotStatements(): o.Statement[] {
      ...
      return this.restoreViewVariable ?
         [this.restoreViewVariable.set(instruction(null, Idf.getCurrentView, [])).toConstDecl()] :
         [];
   }
...
function getPropertyInterpolationExpression(interpolation: Interpolation) {
   switch (getInterpolationArgsLength(interpolation)) {
      case 1:
         return Idf.propertyInterpolate;
      case 3:
         return Idf.propertyInterpolate1;
      case 5:
         return Idf.propertyInterpolate2;
      case 7:
         return Idf.propertyInterpolate3;
      case 9:
         return Idf.propertyInterpolate4;
      case 11:
         return Idf.propertyInterpolate5;
      case 13:
         return Idf.propertyInterpolate6;
      case 15:
         return Idf.propertyInterpolate7;
      case 17:
         return Idf.propertyInterpolate8;
      default:
         return Idf.propertyInterpolateV;
   }
}
...
function getAttributeInterpolationExpression(interpolation: Interpolation) {
   switch (getInterpolationArgsLength(interpolation)) {
      case 3:
         return Idf.attributeInterpolate1;
      case 5:
         return Idf.attributeInterpolate2;
      case 7:
         return Idf.attributeInterpolate3;
      case 9:
         return Idf.attributeInterpolate4;
      case 11:
         return Idf.attributeInterpolate5;
      case 13:
         return Idf.attributeInterpolate6;
      case 15:
         return Idf.attributeInterpolate7;
      case 17:
         return Idf.attributeInterpolate8;
      default:
         return Idf.attributeInterpolateV;
   }
}
...
function getTextInterpolationExpression(interpolation: Interpolation): o.ExternalReference {
   switch (getInterpolationArgsLength(interpolation)) {
      case 1:
         return Idf.textInterpolate;
      case 3:
         return Idf.textInterpolate1;
      case 5:
         return Idf.textInterpolate2;
      case 7:
         return Idf.textInterpolate3;
      case 9:
         return Idf.textInterpolate4;
      case 11:
         return Idf.textInterpolate5;
      case 13:
         return Idf.textInterpolate6;
      case 15:
         return Idf.textInterpolate7;
      case 17:
         return Idf.textInterpolate8;
      default:
         return Idf.textInterpolateV;
   }
}
...
export interface ParseTemplateOptions {
   ...
   /**
    * Whether to always attempt to convert the parsed HTML AST to an Idf AST, despite HTML or i18n
    * Meta parse errors.
    *
    *
    ...
    * nodes to Idf AST nodes in order to provide information for the language service.
    *
    * Note that even when `true` the HTML parse and i18n errors are still appended to the errors
    * output, but this is done after converting the HTML AST to Idf AST.
    */
   alwaysAttemptHtmlToR3AstConversion?: boolean;

   /**
    * Include HTML Comment nodes in a top-level comments array on the returned Idf AST.
    *
...
export function parseTemplate(
   ...
   const { markers, preserveWhitespaces, enableI18nLegacyMessageIdFormat } = options;
   const bindingParser = makeBindingParser(markers);
   ...
   // extraction process (extract-i18n) relies on a raw content to generate
   // message ids
   const i18nMetaVisitor = new I18nMetaVisitor(
      markers, /* keepI18nAttrs */ !preserveWhitespaces,
   ...
   if (!preserveWhitespaces) {
      ...
      // mimic existing extraction process (extract-i18n)
      if (i18nMetaVisitor.hasI18nMeta) {
         rootNodes = html.visitAll(
            new I18nMetaVisitor(markers, /* keepI18nAttrs */ false), rootNodes);
      }
   }

   const { nodes, errors, styleUrls, styles, contentSelectors, commentNodes } = htmlAstToRender3Ast(
...
export function makeBindingParser(
   markers: Markers = DEFAULT_MARKERS): BindingParser {
   return new BindingParser(
      new IvyParser(new Lexer()), markers, elementRegistry, null, []);
}

export function resolveSanitizationFn(context: core.SecurityContext, isAttribute?: boolean) {
   switch (context) {
      case core.SecurityContext.HTML:
         return o.importExpr(Idf.sanitizeHtml);
      case core.SecurityContext.SCRIPT:
         return o.importExpr(Idf.sanitizeScript);
      case core.SecurityContext.STYLE:
         // the compiler does not fill in an instruction for [style.prop?] binding
         // values because the style algorithm knows internally what props are subject
         // to sanitization (only [attr.style] values are explicitly sanitized)
         return isAttribute ? o.importExpr(Idf.sanitizeStyle) : null;
      case core.SecurityContext.URL:
         return o.importExpr(Idf.sanitizeUrl);
      case core.SecurityContext.RESOURCE_URL:
         return o.importExpr(Idf.sanitizeResourceUrl);
      default:
         return null;
   }
}
...
const I18N_CLOSURE_MODE = 'i18nClosureMode';
 ...
 * ```
 * var I18N_1;
 * if (typeof i18nClosureMode !== undefined && iI18nClosureMode) {
 ...
 *
 * ```
 * typeof i18nClosureMode !== undefined && i18nClosureMode
 * ```
 */
function createClosureModeGuard(): o.BinaryOperatorExpr {
   return o.typeofExpr(o.variable(I18N_CLOSURE_MODE))
      .notIdentical(o.literal('undefined', o.STRING_TYPE))
      .and(o.variable(I18N_CLOSURE_MODE));
}
...
export interface ParsedTemplate {
   ...
   /**
    * Any v-content selectors extracted from the template.
    */
   contentSelectors: string[];

   /**
    * Any Idf Comment Nodes extracted from the template when the `collectCommentNodes` parse template
    * option is enabled.
    */
