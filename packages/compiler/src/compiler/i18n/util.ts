...
export function wrapI18nPlaceholder(content: string | number, contextId = 0): string {
  const blockId = contextId > 0 ? `:${contextId}` : '';
  return `${I18N_PLACEHOLDER_SYMBOL}${content}${blockId}${I18N_PLACEHOLDER_SYMBOL}`;
}

export function assembleI18nBoundString(
    strings: string[], bindingStartIndex = 0, contextId = 0): string {
...
export function getSeqNumberGenerator(startsAt = 0): () => number {
...
export function assembleBoundTextPlaceholders(
    meta: i18n.I18nMeta, bindingStartIndex = 0, contextId = 0): Map<string, any[]> {
 ...
 */
export function formatI18nPlaceholderName(name: string, useCamelCase = true): string {
