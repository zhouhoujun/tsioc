...
describe('parser', () => {
    ...
    describe('parse spans', () => {
      it('should record property read span', () => {
        ...
        expect(unparseWithSpan(ast)).toContainEqual(['foo', 'foo']);
        expect(unparseWithSpan(ast)).toContainEqual(['foo', '[nameSpan] foo']);
      });
  
      it('should record accessed property read span', () => {
        const ast = parseAction('foo.bar');
        expect(unparseWithSpan(ast)).toContainEqual(['foo.bar', 'foo.bar']);
        expect(unparseWithSpan(ast)).toContainEqual(['foo.bar', '[nameSpan] bar']);
      });
      ...
      it('should record method call argument span', () => {
        ...
        expect(unparseWithSpan(ast)).toContainEqual(['foo(1 + 2)', '[argumentSpan] 1 + 2']);
      });
      ...
      it('should record property write span', () => {
        ...
        expect(unparseWithSpan(ast)).toContainEqual(['a = b', 'a = b']);
        expect(unparseWithSpan(ast)).toContainEqual(['a = b', '[nameSpan] a']);
      });
  
      it('should record accessed property write span', () => {
        const ast = parseAction('a.b = c');
        expect(unparseWithSpan(ast)).toContainEqual(['a.b = c', 'a.b = c']);
        expect(unparseWithSpan(ast)).toContainEqual(['a.b = c', '[nameSpan] b']);
      });
