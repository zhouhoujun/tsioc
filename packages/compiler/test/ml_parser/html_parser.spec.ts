...
import { ParseError } from '../../src/util';
import { humanizeDom, humanizeDomSourceSpans, humanizeLineColumn, humanizeNodes } from './utils';
    ...
    describe('HtmlParser', () => {
        ...
        describe('parse', () => {
            describe('text nodes', () => {
                ...
                it('should parse text nodes inside <v-template> elements', () => {
                    expect(humanizeDom(parser.parse('<v-template>a</v-template>', 'TestComp'))).toEqual([
                        [html.Element, 'v-template', 0], [html.Text, 'a', 1]
                    ]);
                });
            ...
            describe('elements', () => {
                ...
                it('should parse elements inside  <v-template> elements', () => {
                    expect(humanizeDom(parser.parse('<v-template><span></span></v-template>', 'TestComp')))
                        .toEqual([[html.Element, 'v-template', 0], [html.Element, 'span', 1]]);
                });
            ...
            describe('attributes', () => {
                ...
                it('should parse attributes on <v-template> elements', () => {
                    expect(humanizeDom(parser.parse('<v-template k="v"></v-template>', 'TestComp')))
                        .toEqual([
                            [html.Element, 'v-template', 0],
                            [html.Attribute, 'k', 'v'],
                        ]);
                });
