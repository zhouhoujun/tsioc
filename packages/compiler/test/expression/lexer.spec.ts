...
function expectToken(token: any, index: number) {
  expect(token instanceof Tokenize).toBe(true);
...
function expectErrorToken(token: Tokenize, index: any, message: string) {
  expectToken(token, index);
...
describe('lexer', () => {
  describe('token', () => {
    ...
    it('should tokenize a string', () => {
      const tokens: Tokenize[] = lex('j-a.bc[22]+1.3|f:\'a\\\'c\':"d\\"e"');
      expectIdentifierToken(tokens[0], 0, 'j');
      expectOperatorToken(tokens[1], 1, '-');
    ...
    it('should tokenize number with exponent', () => {
      let tokens: Tokenize[] = lex('0.5E-10');
      expect(tokens.length).toEqual(1);
