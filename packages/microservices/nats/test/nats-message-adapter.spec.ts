...
import { NatsMessageAdapter } from '../src/server/message-adapter';

describe('NATS message adapter auth-facing reads', () => {
    it('reads authorization headers, query token, body payload, and writes status state', () => {
        const request = {
            headers: { authorization: 'Bearer nats-token', 'x-test': '1' },
        ...
        const adapter = new NatsMessageAdapter(request, {} as any);
        adapter.setStatus(202, 'Accepted');
        adapter.setHeader('x-auth', 'ok');
        adapter.write({ ok: true });

        expect(adapter.read('headers', 'authorization')).toBe('Bearer nats-token');
