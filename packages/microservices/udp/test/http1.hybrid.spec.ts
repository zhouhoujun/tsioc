...
import { UdpClient, withUdpTransport, useUdpTransport } from '../src';
import { DeviceController } from './controller';
import { catchError, lastValueFrom, of } from 'rxjs';
import expect = require('expect');

const HTTP_PORT = 21323;
...
class UdpHttpHybridModule { }

describe('Udp hybrid HTTP server and Udp client', () => {
    ...
    const sendProto = (pattern: any, options: any = {}) => {
        return lastValueFrom(udpClient.send(pattern, options).pipe(catchError(err => of(err))));
    };

    before(async () => {
        ctx = await Application.run(UdpHttpHybridModule);
    ...
    it('resolves clients', () => { expect(httpClient).toBeDefined(); expect(udpClient).toBeDefined(); });
