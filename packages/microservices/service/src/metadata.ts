...
import { createRouteHandler } from './router/route.handler';
    ...
    design: {
        method: (typeRef, ctx) => {
            ...
            defines.forEach(def => {
                ...
                const prefix = metadata.prefix ? normalize(metadata.prefix) : '';
...
export const Handle: Handle = createDecorator<HandleMetadata<any>>('Handle', {
    ...
    design: {
        ...
        afterAnnoation: (typeRef, ctx) => {
            ...
            if (!router) throw new Exception(getTypeName(typeRef.type) + ' has not registered!');
            if (!(router instanceof Router)) throw new Exception(getTypeName(router) + ' is not router!');

            router.use({
                prefix: mapping?.prefix ? normalize(joinPath(mapping.prefix, mapping.version)) : '',
...
export interface Controller {
     ...
     *
     * @param {string} route route sub path.
     * @param {AbstractType<Router>} [parent] the router for the controller.
     */
     ...
     * controller decorator. define the controller method as an route.
     *
     * @param {Omit<RouteMappingOptions, 'route' | 'response'>} [metadata] route metadata.
     */
    (metadata: Omit<RouteMappingOptions, 'route' | 'response'>): ClassDecorator;
}

/**
 * Controller decorator
 */
export const Controller: Controller = createMappingDecorator('Controller', true);
...
export interface HeadDecorator {
     ...
     *
     * @param {string} route route sub path.
     * @param options route metadate options.
     */
    (route: string, options: RouteOptions): MethodDecorator;
}
