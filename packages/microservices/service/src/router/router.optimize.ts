...
import { Pattern, PatternFormatter, BadRequestException, NotFoundException, isResponseStateCapableMessageAdapter, RequestHandler, Transport, RequestContext, ReadableLike, Incoming, UrlIncoming, TopicIncoming, REQUEST, RESPONSE } from '@tsdi/common'
...
import { RouteHandler, createRouteHandler } from './route.handler';
...
export class OptimizedRouter extends Router<RouteHanlder> implements OnDestroy {
    ...
    doHandle(req: ReadableLike<Incoming>, context: RequestContext, notFound?: () => Observable<any>): Observable<any> {
        ...
        if (res && (res.headersSent || (res.statusCode && !(res.error instanceof NotFoundException)))) return of(null);
    ...
    protected parse(route: Route): HandlerFn | undefined {
        if (route.handler) {
            ...
            return (i, c) => from(this.redirect(i as any, to))
        }
    }
        ...
        if (this.microservice) {
            ...
            if (routes.length > 1) {
                ...
                const handles = routes.map(route => {
                    return (input: ReadableLike<Incoming>, context?: RequestContext) => {
