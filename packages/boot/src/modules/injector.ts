import { Token, lang, SymbolType, Type, IInjector, IProvider, isContainer, ProviderType, isNil, getTokenKey, InstFac, isClass, Provider, isPlainObject, InjectorImpl } from '@tsdi/ioc';
import { ROOT_INJECTOR } from '../tk';
import { IModuleInjector, IModuleProvider, ModuleRef } from './ModuleRef';
...
export class ModuleProviders extends Provider implements IModuleProvider {
    ...
    export(type: Type) {
        const state = this.getContainer().getTypeReflects();
        if (!state.hasRegister(type) && !this.mdInjector.has(type)) {
