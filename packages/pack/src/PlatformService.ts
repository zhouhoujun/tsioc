import { Injectable, ObjectMap, isArray, isString, lang, isFunction, ArgumentException } from '@tsdi/ioc';
...
export class PlatformService {
    ...
    async getFiles(express: Src, options: any, filter?: any, mapping?: (filename: string) => string): Promise<string[]> {
        if (!(isString(express) || isArray(express))) {
            throw new ArgumentException('input express param type error!');
        }
