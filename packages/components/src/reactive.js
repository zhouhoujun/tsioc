...
exports.canReactive = canReactive;
...
function isReactive(target) {
    return (0, ioc_1.isObject)(target) && target[REACT_FlAG];
}
function canReactive(target) {
    // 如果target已经是响应式的，直接返回
    if (!target || !(0, ioc_1.isObject)(target) || target[REACT_FlAG] || target[effect_1.noReact]) {
        return false;
    }
...
function reactive(target, effect, computeds) {
    // 直接返回，不进行代理
    ...
    const proxy = new Proxy(target, {
        get(target, key, receiver) {
            ...
            if (isComputing && currentComputedKey) {
                ...
                if (computedDep?.dependencies?.includes(key)) {
                    // 只对依赖属性进行追踪
        ...
        deleteProperty(target, key) {
            const hadKey = (0, ioc_1.hasOwn)(target, key);
...
// 处理计算属性访问
function handleComputedProperty(target, computed, effect) {
    // 初始化缓存
        ...
        try {
            ...
            if (computed?.cache !== false) {
                ...
                if (computed?.dependencies) {
                    computed.dependencies.forEach((dep) => newDeps.add(dep));
                }
...
// 清除计算属性缓存
function clearComputedCache(target, key) {
    if (!computedCache.has(target))
        return;
    const targetCache = computedCache.get(target);
    if (key) {
