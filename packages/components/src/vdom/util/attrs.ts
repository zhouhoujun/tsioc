import { isProceduralRenderer, ProceduralRenderer, RElement, Renderer } from '../renderer';
import { CharCode } from '../../util/char';
import { AttributeMarker, TAttributes } from '../node';
...
export function setUpAttributes(renderer: Renderer, native: RElement, attrs: TAttributes): number {
    ...
    while (i < attrs.length) {
        ...
        if (typeof value === 'number') {
            ...
            isProc ?
                (renderer as ProceduralRenderer).setAttribute(native, attrName, attrVal, namespaceURI) :
            ...
            if (isAnimationProp(attrName)) {
                if (isProc) {
                    (renderer as ProceduralRenderer).setProperty(native, attrName, attrVal);
                }
            } else {
                isProc ?
                    (renderer as ProceduralRenderer).setAttribute(native, attrName, attrVal as string) :
