import { AttributeMarker, TAttributes, TNode, TNodeType } from './node';
import { CssSelector, CssSelectorList, SelectorFlags } from './projection';
import { classIndexOf } from './styling/differ';
import { templateTag } from './types';
import { isNameOnlyAttributeMarker } from './util/attrs';
...
export function isInlineTemplate(tNode: TNode): boolean {
    return tNode.type === TNodeType.Container && tNode.tagName !== templateTag;
}
 ...
 * different from templateTag (value different from templateTag indicates that a
...
export function isNodeMatchingSelectorList(
    tNode: TNode, selector: CssSelectorList, isProjectionMode = false): boolean {
    for (let i = 0; i < selector.length; i++) {
 ...
 */
export function extractAttrsAndClassesFromSelector(selector: CssSelector): { attrs: string[], classes: string[] } {
    const attrs: string[] = [];
