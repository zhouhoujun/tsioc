...
import { LQueries, LQuery, TQueryMetadata, QueryFlags, TQueries, TQuery } from '../interfaces/query';
...
class TQueryMetadata_ implements TQueryMetadata {
    constructor(
        public predicate: Token | string[], public flags: QueryFlags,
...
class TQuery_ implements TQuery {
    ...
    constructor(public metadata: TQueryMetadata, nodeIndex = -1) {
    ...
    private matchTNode(tView: TView, tNode: TNode): void {
        ...
        if (Array.isArray(predicate)) {
            ...
            if ((predicate as any) === TemplateRef) {
        ...
        if (nodeMatchIdx !== null) {
            ...
            if (read !== null) {
                if (read === ElementRef || read === ViewContainerRef ||
                    read === TemplateRef && (tNode.type & TNodeType.Container)) {
...
function materializeViewResults<T>(
    ...
    if (lQuery.matches === null) {
        ...
        for (let i = 0; i < tQueryMatches.length; i += 2) {
            ...
            if (matchedNodeIdx < 0) {
                // we at the <template> marker which might have results in views created based on this
                // <template> - those results will be in separate views though, so here we just leave
...
export function ɵɵviewQuery<T>(
    predicate: Token | string[], flags: QueryFlags, read?: any): void {
