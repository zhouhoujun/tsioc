import { IActionSetup, DecoratorProvider, isFunction, isDefined, isTypeObject, lang, isArray, chain, isNullOrUndefined } from '@tsdi/ioc';
...
export const ResolveTemplateHanlde = async function (ctx: IComponentContext, next: () => Promise<void>): Promise<void> {
    ...
    let pCtx = compPdr.createTemplateContext(ctx.injector, {
        parent: ctx,
        template: ctx.getAnnoation().template
    });
