...
import { TransportContext, ResponseStatusFormater } from '@tsdi/endpoints';
...
export class NodeResponseStatusFormater extends ResponseStatusFormater {
    ...
    protected formatWithColor(withColor: boolean, ctx: TransportContext, hrtime?: [number, number]) {
        if (hrtime) {
            const [status, message] = ctx instanceof TransportContext ? this.formatStatus(ctx, withColor) : this.formatState(ctx, withColor);
    ...
    private formatStatus(ctx: TransportContext, withColor: boolean): [StatusCode, string] {
